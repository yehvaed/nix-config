...
inherit (builtins) head listToAttrs readFile map baseNameOf hasAttr getAttr replaceStrings filter toFile;
...
let
  findPipe = item: getAttr "pipe" (
    findFirst ({ cond, ... }: cond item) null pipeMaps
  );
...
mergeItem = itemFactory: item:
  item // (itemFactory item);

files = listFilesRecursive ./config;

extend = f: { pkgs, lib, ... }@inputs: f { 
  mapToConfig = matchPipe [
    ...
    { 
      pipe = [
        ...
        (mergeItem ({ pname, ... }: { 
          listedConfigFiles = filter (file: hasPrefix pname (baseNameOf file)) files;
        }))
        ...
        (mergeItem ({ configFile, ... }: {
          config = if hasSuffix ".nix" configFile then import (configFile) (inputs) 
            else readFile (configFile);
        }))
      ];

      cond = { pname, ...}@plugin: filter (file: hasPrefix pname (baseNameOf file)) files != []; 
    }
